...
// ============================================================================
// app/contactos/CountryCombobox.tsx — Buscador de País con Prefijo
//
...
export function CountryCombobox({ value, onChange }: CountryComboboxProps) {
  ...
  return (
    <div ref={containerRef} className="relative">
      {/* Botón: muestra el prefijo del país seleccionado */}
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1 rounded-md border border-zinc-700 bg-zinc-800 px-2 py-0.5 text-xs font-mono text-zinc-300 transition-colors hover:border-zinc-600 hover:text-zinc-100"
        title={selected?.name}
      >
              ...
              filtered.map((c) => (
                <li key={c.code}>
                  <button
                    ...
                    className={`flex w-full items-center justify-between px-3 py-1.5 text-xs transition-colors hover:bg-zinc-800 ${
